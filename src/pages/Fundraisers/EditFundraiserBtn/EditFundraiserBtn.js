...
const EditFundraiserBtn = (props) => {
  ...
  const handleFundraiser = async () => {
    ...
    if (!newFundraiser.fund_name || !newFundraiser.fund_description || !newFundraiser.fund_location || !newFundraiser.fund_date) {
      alert('Please provide a fundraiser name, description, location, and date.');
      return;
    }
    ...
    try {
      console.log('Attempting to edit fundraiser with ID:', props.fund_id);
      const response = await axios.put(
        `http://localhost:8081/fundraisers/edit/${props.fund_id}`,
        fundraiserData
      );
